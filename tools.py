...
def hz_channelorder():
    ...
    try:
        # 正式库
        conn = pyodbc.connect(r'DRIVER={SQL Server};SERVER=localhost;DATABASE=testok;UID=KFT;PWD=test')
        # 测试库
        # conn=pyodbc.connect(r'DRIVER={SQL Server};SERVER=localhost;DATABASE=testok_TEST;UID=test;PWD=test')
        cur = conn.cursor()
    except pyodbc.OperationalError as reason1:
        mes.showerror("错误提示", "找不到服务器,原因是:%s" % str(reason1))
    except pyodbc.InterfaceError as reason:
...
def hy_channelorder():
    ...
    model_select()
    # 正式库
    con = pyodbc.connect(r'DRIVER={SQL Server};SERVER=localhost;DATABASE=testok;UID=test;PWD=test')
    curo = con.cursor()
    if mo == '':
...
def hz_finishorder():
    ...
    if HZ_billno == '':
        ...
        if answer is False:
            ...
            urls = ['http://localhost/api/ApplyNew_Completed.do',
                    'http://localhost/api/Repair_Completed.do',
                    'http://localhost/api/Exchange_Completed.do']
            # print(billnos)
            # print(type(billnos))
            infos = ''
            for billno in billnos:
...
# 浩优结单
# @model_select #hy_finishorder=model_select(hy_finishorder)
# @model_select
def hy_finishorder():
    showbill.delete(1.0, "end")
    model_select()
                ...
                for billno in billnos:
                    ...
                    if test is None:
                        ...
                        if state_id in states:
                            ...
                            update_sql = "update mx_repair set billstate=130000008,remark=remark+char(13)+'工具结单' where billno='%s'" % str(billno)
                            # update_sql2 = "update Family_Base_Service_Order set CRMCheck=110000006 where CRMID='%s'" % str(billno)
                            curo.execute(update_sql)
                            # curo.execute(update_sql2)
                            con.commit()
...
def save_sqllog(sqlinfo):
    ...
    f = open("log/" + log_head + "SQLlog.txt", "a")
    now = str(datetime.datetime.now())
    f.write(now + ' : ' + sqlinfo + "\n")
    f.close()
...
def dumpexecl(final_sql):
    ###导出数据连接镜像数据库
    try:
        conn = pyodbc.connect(r'DRIVER={SQL Server Native Client 10.0};SERVER=localhost;DATABASE=testok_snap;UID=test;PWD=test')
        cur = conn.cursor()
...
def get_bad_order():
    ...
    # global bad_order
    try:
        conn = pyodbc.connect(r'DRIVER={SQL Server Native Client 10.0};SERVER=localhost;DATABASE=wmsinput;UID=test;PWD=test')
        cur = conn.cursor()
...
def output_bad_order():
    ...
    # canvas_abnormal.create_text(92, 142, text=orders[0][0],fill="orange",font=("Verdana",10))
    # canvas_abnormal.create_text(216, 142, text=orders[0][1], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(349, 142, text=orders[0][2], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(482, 142, text=orders[0][3], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(92, 187, text=orders[1][0], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(216, 187, text=orders[1][1], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(349, 187, text=orders[1][2], fill="orange", font=("Verdana", 10))
    # canvas_abnormal.create_text(482, 187, text=orders[1][3], fill="orange", font=("Verdana", 10))
    y = 142
    try:
        for i in range(5):
            canvas_abnormal.create_text(92, y, text=orders[i][0], fill="orange", font=("Verdana", 10))
            canvas_abnormal.create_text(216, y, text=orders[i][1], fill="orange", font=("Verdana", 10))
            canvas_abnormal.create_text(349, y, text=orders[i][2], fill="orange", font=("Verdana", 10))
            canvas_abnormal.create_text(482, y, text=str(orders[i][3]) + '分钟', fill="orange", font=("Verdana", 10))
            y = y + 44
        raise IndexError
    except IndexError:
        pass
...
def select_reason():
    ...
    if bad_billno[0:2] == 'KH':
        ...
        conn = pyodbc.connect(r'DRIVER={SQL Server Native Client 10.0};SERVER=localhost;DATABASE=testok;UID=test;PWD=test')
        cur = conn.cursor()
        ...
        info = "单据未选择或输入错误！"
    print(info)
    canvas_abnormal.create_text(290, 480, text=info, fill="orange", font=("Verdana", 13))
    mes.showinfo("提示", info)
    return solution_id


# if count == 1:
# 	canvas_abnormal.create_text(290,465,text=info,fill="orange",font=("Verdana", 13))
# elif count ==2:
# 	canvas_abnormal.create_text(290, 490, text=info, fill="orange", font=("Verdana", 13))
# else:
# 	canvas_abnormal.create_text(290, 525, text=info, fill="orange", font=("Verdana", 13))
# count+=1
...
def driver_link():
    url = "https://msdn.microsoft.com/en-us/data/ff658533.aspx"
    webbrowser.open(url)
...
###########自动邮件###########
canvas_email = tk.Canvas(tab3, height=585, width=580)
canvas_email.pack()
# canvas_email.create_rectangle(0,0,580,585,fill="white")
img_3 = tk.PhotoImage(file="image/email1.png")
canvas_email.create_image(0, 200, anchor=tk.NW, image=img_3)
...
canvas_export = tk.Canvas(tab7, height=585, width=580)
canvas_export.pack()
canvas_export.create_rectangle(0, 0, 580, 585, fill="skyblue")
canvas_export.create_text(290, 35, text="数据导出", fill='white', font=('Comic Sans MS', 25))
canvas_export.create_rectangle(0, 75, 580, 78, fill="white")
canvas_export.create_rectangle(0, 251, 580, 254, fill="white")
canvas_export.create_rectangle(0, 428, 580, 431, fill="white")
##时间类型下拉框
canvas_export.create_text(50, 111, text="日期类型：", fill='black', font=('Comic Sans MS', 10))
...
##年下拉框
canvas_export.create_text(220, 111, text="S年:", fill='black', font=('Comic Sans MS', 10))
syear = tk.StringVar()
...
##月下拉框
canvas_export.create_text(345, 111, text="S月:", fill='black', font=('Comic Sans MS', 10))
smonth = tk.StringVar()
...
##日下拉框
canvas_export.create_text(465, 111, text="S日:", fill='black', font=('Comic Sans MS', 10))
sday = tk.StringVar()
...
##状态下拉框
canvas_export.create_text(50, 151, text="单据状态：", fill='black', font=('Comic Sans MS', 10))
...
##年下拉框
canvas_export.create_text(220, 151, text="E年:", fill='black', font=('Comic Sans MS', 10))
eyear = tk.StringVar()
...
##月下拉框
canvas_export.create_text(345, 151, text="E月:", fill='black', font=('Comic Sans MS', 10))
emonth = tk.StringVar()
...
##日下拉框
canvas_export.create_text(465, 151, text="E日:", fill='black', font=('Comic Sans MS', 10))
eday = tk.StringVar()
...
canvas_export.create_text(90, 190, text="区域代码", fill='black', font=('Comic Sans MS', 10))
areacode = tk.Entry(tab7)
areacode.place(x=120, y=180)
canvas_export.create_text(290, 190, text="机器P码", fill='black', font=('Comic Sans MS', 10))
...
canvas_export.create_text(90, 290, text="案例城市", fill='black', font=('Comic Sans Ms', 10))
city = tk.Entry(tab7)
city.place(x=120, y=280)

canvas_export.create_text(290, 290, text="案例名称", fill='black', font=('Comic Sans Ms', 10))
...
canvas_export.create_text(290, 460, text="Tips:多个代码机型(注意是P码)请用英文逗号分开,", fill='white', font=('Comic Sans MS', 15))
canvas_export.create_text(290, 490, text="为便于输入，案例城市，案例名称请用中文逗号分开", fill='white', font=('Comic Sans MS', 15))
canvas_export.create_text(290, 520, text="出于信息安全，数据量限制5000条，请不要超过此数量", fill='white', font=('Comic Sans MS', 15))

#####################################################异常数据V1###############################################

# canvas_abnormal=tk.Canvas(tab8,height=585,width=580)
# canvas_abnormal.pack()
# canvas_abnormal.create_rectangle(0,0,580,385,fill="Khaki")
# canvas_abnormal.create_rectangle(0,385,580,585,fill="DarkKhaki")
# canvas_abnormal.create_text(290,35,text="Bad Order",fill='White',font=('Comic Sans MS',25))
...
# canvas_abnormal.create_polygon(points, outline="#476042",
# 			fill='yellow', width=3)
# canvas_abnormal.create_polygon(points_8, outline="#476042",
# 			fill='yellow', width=3)
...
canvas_abnormal = tk.Canvas(tab8, height=585, width=580)
canvas_abnormal.pack()
dmimg = tk.PhotoImage(file="image/orderDM.png")
canvas_abnormal.create_image(0, 0, anchor=tk.NW, image=dmimg)
canvas_abnormal.create_text(290, 35, text="数据监控", fill="#F4FFEC", font=('Verdana', 25))
##画横线
canvas_abnormal.create_rectangle(30, 75, 550, 78, fill="white")
canvas_abnormal.create_rectangle(30, 120, 550, 122, fill="white")
canvas_abnormal.create_rectangle(30, 165, 550, 167, fill="white")
canvas_abnormal.create_rectangle(30, 210, 550, 212, fill="white")
canvas_abnormal.create_rectangle(30, 255, 550, 257, fill="white")
canvas_abnormal.create_rectangle(30, 300, 550, 302, fill="white")
canvas_abnormal.create_rectangle(30, 345, 550, 348, fill="white")
##画纵线
canvas_abnormal.create_rectangle(150, 78, 152, 348, fill="white")
canvas_abnormal.create_rectangle(283, 78, 285, 348, fill="white")
canvas_abnormal.create_rectangle(416, 78, 418, 348, fill="white")
# 画首行
canvas_abnormal.create_text(92, 97.5, text="单据编号", fill="white", font=("Verdana", 15))
canvas_abnormal.create_text(216, 97.5, text="单据类型", fill="white", font=("Verdana", 15))
canvas_abnormal.create_text(349, 97.5, text="单据状态", fill="white", font=("Verdana", 15))
canvas_abnormal.create_text(482, 97.5, text="超时时间", fill="white", font=("Verdana", 15))
...
######################################################帮助说明###################################################
canvas_help = tk.Canvas(tab5, height=585, width=580)
canvas_help.pack()
canvas_help.create_rectangle(0, 0, 580, 585, fill='white')
help_image = tk.PhotoImage(file="image/helpphoto.png")
canvas_help.create_image(10, 20, anchor=tk.NW, image=help_image)
# canvas_help.create_window(50,15,tags='BTN1',window=tk.Button(tab5,text=u'侦听',width=8,command=lambda: mes.showinfo('1','1')))
###########关于###########
...
fileMenu_s.add_command(label="打开")
fileMenu_s.add_separator()
fileMenu_s.add_command(label="no define")
...
fileMenu_f.add_separator()
fileMenu_f.add_command(label="驱动链接", command=driver_link)
fileMenu_f.add_separator()
